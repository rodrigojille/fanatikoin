...
async function main() {
  ...
  // Use normal dynamic gas and default pending nonce
  const feeData = await provider.getFeeData();
  const baseGasPrice = feeData.gasPrice || ethers.parseUnits('10', 'gwei');
  const gasPrice = (baseGasPrice * 15n) / 10n; // 1.5x buffer
  ...
  console.log(`Buffered gas price (1.5x): ${ethers.formatUnits(gasPrice, 'gwei')} gwei`);
  ...
  try {
    ...
    console.error("❌ Error sending Puebla FC token creation tx:", err);
  }
}
